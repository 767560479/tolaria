...
> **Product note (fork):** Tolaria no longer ships a vault MCP server or in-app CLI coding agents. In-app AI is direct model chat (`stream_ai_model` / AiWorkspace) plus optional AI-generated Git commit messages.
...
- Day-to-day feature work lands on the long-lived **`alpha`** branch (`git push origin alpha`). `main` is the **Stable baseline**; promote by merging `alpha` → `main`, then pushing a `stable-vYYYY.M.D` tag. No PR branch flow is required.
- Commit every 20–30 min: `feat:`, `fix:`, `refactor:`, `test:`, `docs:`
- Pre-commit is a lightweight lint gate only. Pre-push runs the full check suite (build + tests + coverage + core Playwright smoke + CodeScene), preferably locally (or optional Chunk sidecars if configured). Keep local Playwright mainly for authoring and focused reproduction.
- GitHub Actions owns the release path (ADR 0178): push to **`alpha`** publishes an Alpha prerelease; push tag **`stable-v*`** publishes a Stable release. Ordinary pushes to `main` do not publish. No signing secrets are required. Quality gates remain local/husky.
- **A feature task is NOT done until `git push origin alpha` succeeds** (unless the task is explicitly a Stable promotion). If the hook blocks: read the error, fix it (clippy, tests, CodeScene, build), commit the fix, push again. **⛔ NEVER use `--no-verify`**
...
**Before the final push to the task's release branch (`origin/alpha` for features, or `origin/main` for Stable promotion commits):** run `mcp__codescene__analyze_change_set` with `base_ref=origin/alpha` (or `origin/main` for Stable-only work). This is Tolaria's PR-preflight equivalent: every affected file must be improved or stable, and the overall quality gate must pass. Refactor and repeat if any file is degraded.
