...
- Current Windows-only `release.yml` does **not** require telemetry secrets
- Optional: set `VITE_SENTRY_DSN`, `SENTRY_DSN`, `VITE_POSTHOG_KEY`, `VITE_POSTHOG_HOST` later if you reintroduce signed/telemetry-enabled packaged builds
- Without those values, Settings toggles may still appear but PostHog/Sentry may not initialize in the packaged app
