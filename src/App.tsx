...
import { useOpenMarkdownFromShell } from './hooks/useOpenMarkdownFromShell'
...
import { selectionShowsNoteList, VAULT_ROOT_SELECTION } from './utils/sidebarSelectionVisibility'
...
const DEFAULT_SELECTION: SidebarSelection = VAULT_ROOT_SELECTION
...
function MainApp({ noteWindowParams }: { noteWindowParams: NoteWindowParams | null }) {
  ...
  const {
    ...
    aiWorkspaceWindow,
    closeAIChat,
    modelSelectorAvailable: aiFeaturesEnabled
      && ((settings.ai_model_providers?.length ?? 0) > 0 || aiFeaturesEnabled),
    openAIChat,
  ...
  const quickPromptTargetReady = resolveAiTargetReadiness(quickPromptTarget, {
    settingsLoaded,
  }).canQueuePrompt
  ...
  const {
    ...
    noteListVisible: viewModeNoteListVisible,
    pdfExportRef,
    rawToggleRef,
    sidebarVisible,
  ...
  const noteListVisible = viewModeNoteListVisible || selectionShowsNoteList(effectiveSelection)
  ...
  const handleCheckForUpdates = useCallback(async () => {
    ...
    if (result.kind === 'up-to-date') {
      setToastMessage(translate(appLocale, 'update.upToDate'))
    } else if (result.kind === 'available') {
      setToastMessage(translate(appLocale, 'update.availableToast', { version: result.displayVersion }))
    } else {
      setToastMessage(result.message)
    }
  }, [appLocale, updateActions])
