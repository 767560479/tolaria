...
export type AiTarget = AiModelTarget
...
export const MISSING_MODEL_TARGET_ID = 'model:__missing__/__missing__'
...
const MISSING_MODEL_PROVIDER: AiModelProvider = {
  id: '__missing__',
...
export function createMissingModelTarget(): AiModelTarget {
  const model = MISSING_MODEL_PROVIDER.models[0]
  return {
    kind: 'api_model',
    provider: MISSING_MODEL_PROVIDER,
    model,
    id: MISSING_MODEL_TARGET_ID,
    label: 'No model configured',
    shortLabel: 'None',
  }
}

export function isMissingModelTarget(target: AiTarget): boolean {
  return target.id === MISSING_MODEL_TARGET_ID
}
...
export function resolveAiTarget(settings: Settings): AiTarget {
  ...
  const targets = configuredModelTargets(providers)
  const storedTarget = resolveStoredModelTarget(settings.default_ai_target, targets)
  if (storedTarget) return storedTarget

  return targets[0] ?? createMissingModelTarget()
}

export function targetAgent(): AiAgentId {
  return DEFAULT_AI_AGENT
}

function resolveStoredModelTarget(
  storedTarget: string | null | undefined,
  targets: AiModelTarget[],
): AiModelTarget | null {
  const target = storedTarget?.trim()
  if (!target || target.startsWith(AI_TARGET_PREFIX_AGENT)) return null

  return targets.find((candidate) => candidate.id === target) ?? null
}
...
export function resolveAiTargetReadiness(
  ...
  if (isMissingModelTarget(target)) return aiTargetReadinessResult('missing')
  return aiTargetReadinessResult('ready')
}
...
export function aiTargetReady(
  ...
  return resolveAiTargetReadiness(target, options).ready
}
...
export function aiTargetCanQueuePrompt(
  ...
  return resolveAiTargetReadiness(target, options).canQueuePrompt
}
