...
import { isMissingModelTarget } from './aiTargets'
...
function blockUnavailableTarget(runtime: AiAgentSessionRuntime, context: AgentExecutionContext, prompt: PendingUserPrompt): void {
  trackAiAgentMessageBlocked(context.agent, 'agent_unavailable')
  appendLocalResponse(
    runtime.setMessages,
    prompt,
    createTranslator(context.locale ?? 'en')('ai.panel.noModelConfigured'),
  )
}
...
async function streamWithSelectedTarget({
  ...
  if (context.target?.kind !== 'api_model' || isMissingModelTarget(context.target)) return
...
export async function sendAgentMessage({
  ...
  if (!context.ready || !context.target || isMissingModelTarget(context.target)) {
    blockUnavailableTarget(runtime, context, normalizedPrompt)
    return
  }
