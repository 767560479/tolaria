...
const emptySettings: Settings = {
  ...
  picgo_server_url: null,
  picgo_server_token: null,
}
...
describe('SettingsPanel', () => {
  ...
  it('separates local models and API models in AI settings', async () => {
    render(
      <SettingsPanel
        open={true}
    ...
    expect(screen.getByTestId('settings-default-ai-target-empty')).toBeInTheDocument()
  ...
  it('shows manual GitHub update guidance instead of channel controls', () => {
    render(
      <SettingsPanel open={true} settings={emptySettings} onSave={onSave} onClose={onClose} />
    )

    expect(screen.getByTestId('settings-manual-update-checks')).toBeInTheDocument()
    expect(screen.queryByTestId('settings-release-channel')).not.toBeInTheDocument()
    expect(screen.queryByRole('switch', { name: /updates automatically/i })).not.toBeInTheDocument()
  })

  it('forces automatic update checks off when saving settings', () => {
    render(
      <SettingsPanel open={true} settings={emptySettings} onSave={onSave} onClose={onClose} />
    )
  ...
  it('anchors the default model dropdown with the popper strategy', () => {
    const settingsWithModel: Settings = {
      ...
      default_ai_target: 'model:openai-test/gpt-4o',
    }
    render(
      <SettingsPanel open={true} settings={settingsWithModel} onSave={onSave} onClose={onClose} />
    )
  ...
  it('keeps keyboard opening enabled for the default model dropdown', () => {
    const settingsWithModel: Settings = {
      ...emptySettings,
      ai_model_providers: [{
        id: 'openai-test',
  ...
  it('preserves an existing release_channel value when saving unrelated settings', () => {
  ...
  it('does not trap focus away from a portaled settings dropdown', () => {
    ...
    const option = screen.getByRole('option', { name: /gpt-4o/i })
    act(() => {
      option.focus()
    })
