...
import { createMissingModelTarget, isMissingModelTarget, type AiTarget } from '../lib/aiTargets'
...
export function firstTarget(
  ...
  const selectedDefault = defaultTarget && !isMissingModelTarget(defaultTarget)
    ? targets.find((target) => target.id === defaultTarget.id)
    : undefined
