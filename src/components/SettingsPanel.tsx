...
function createSettingsDraft(
  ...
  return {
    ...
    picgoServerUrl: settings.picgo_server_url ?? '',
    picgoServerToken: settings.picgo_server_token ?? '',
...
function buildSettingsFromDraft(settings: Settings, draft: SettingsDraft): Settings {
  const nextSettings = {
    ...
    picgo_server_url: normalizePicgoServerUrl(draft.picgoServerUrl),
    picgo_server_token: normalizePicgoServerToken(draft.picgoServerToken),
  }
...
type SettingsPanelInnerProps = Omit<SettingsPanelProps, 'open' | 'explicitOrganizationEnabled' | 'isGitVault' | 'vaultPath'> & {
  initialSectionId: string | null
  locale: AppLocale
  systemLocale: AppLocale
  isGitVault: boolean
...
function SettingsBodyFromDraft({
  ...
  return (
    <SettingsBody
      ...
      picgoServerUrl={draft.picgoServerUrl}
      setPicgoServerUrl={(value) => updateDraft('picgoServerUrl', value)}
      picgoServerToken={draft.picgoServerToken}
      setPicgoServerToken={(value) => updateDraft('picgoServerToken', value)}
      vaults={vaults}
      defaultWorkspacePath={defaultWorkspacePath}
      {...{ onRemoveVault, onReorderVaults, onSetDefaultWorkspace, onUpdateWorkspaceIdentity }}
...
function SettingsContentSections({
  ...
  return (
    <>
      ...
      <SettingsSection id={SETTINGS_SECTION_IDS.gallery}>
        <GallerySettingsSection
          t={t}
          picgoServerUrl={picgoServerUrl}
          setPicgoServerUrl={setPicgoServerUrl}
          picgoServerToken={picgoServerToken}
          setPicgoServerToken={setPicgoServerToken}
        />
      </SettingsSection>
    </>
  )
  ...
  return (
    <>
      ...
      <SettingsGroup>
        ...
        <SettingsRow
          label={t('settings.manualUpdateChecks')}
          description={t('settings.manualUpdateChecksDescription')}
        >
          <span className="text-xs text-muted-foreground" data-testid="settings-manual-update-checks">
            {t('settings.manualUpdateChecksHint')}
          </span>
        </SettingsRow>
      </SettingsGroup>
    </>
...
function buildDefaultAiTargetOptions(
  ...
  return configuredModelTargets(providers).map((target) => ({
    value: target.id,
...
>) {
  ...
  const targetOptions = buildDefaultAiTargetOptions(aiModelProviders, t)
        ...
        <>
          <SettingsGroup>
            <SettingsRow
              ...
              description={renderDefaultAiTargetSummary(selectedTarget, t)}
              controlWidth="wide"
            >
              {targetOptions.length > 0 ? (
                <SelectControl
                  ...
                  onValueChange={setDefaultAiTarget}
                ...
                <span
                  ...
                  data-testid="settings-default-ai-target-empty"
                >
                  {t('settings.aiAgents.noConfiguredTargets')}
                </span>
              )}
            </SettingsRow>
          </SettingsGroup>
...
function renderDefaultAiTargetSummary(target: ReturnType<typeof resolveAiTarget>, t: Translate): string {
  if (target.kind !== 'api_model') {
    return t('settings.aiAgents.noConfiguredTargets')
  }
