...
describe('Sidebar', () => {
  it('renders without top-nav filters', () => {
  ...
  describe('type visibility via visible property', () => {
    ...
    it('does not affect other sidebar chrome when sections are hidden', () => {
      const entries: VaultEntry[] = [
        ...mockEntries,
  ...
  it('does not render Inbox / All Notes / Archive top nav', () => {
    render(<Sidebar entries={[]} selection={defaultSelection} onSelect={() => {}} />)
  ...
  it('renders a tags section for notes with tags', () => {
    ...
    render(<Sidebar entries={tagged} selection={defaultSelection} onSelect={() => {}} />)
    expect(screen.getByTestId('sidebar-tags')).toBeInTheDocument()
    expect(screen.getByText('ai')).toBeInTheDocument()
  })

  it('calls onSelect with tag selection when clicking a tag', () => {
    const onSelect = vi.fn()
    const tagged: VaultEntry[] = [{
      ...mockEntries[0],
      path: '/vault/tagged.md',
    ...
    render(<Sidebar entries={tagged} selection={defaultSelection} onSelect={onSelect} />)
    fireEvent.click(screen.getByText('ai'))
    expect(onSelect).toHaveBeenCalledWith({ kind: 'tag', tag: 'ai' })
  })

  it('hides Inbox when explicit organization is disabled (top nav removed)', () => {
    render(<Sidebar entries={[]} selection={defaultSelection} onSelect={() => {}} />)
    expect(screen.queryByText('Inbox')).not.toBeInTheDocument()
    expect(screen.queryByTestId('sidebar-top-nav')).not.toBeInTheDocument()
  })

  it('does not render All Notes / Archive after top-nav removal', () => {
    render(<Sidebar entries={mockEntries} selection={defaultSelection} onSelect={() => {}} />)
    expect(screen.queryByTestId('sidebar-top-nav')).not.toBeInTheDocument()
    expect(screen.queryByText('All Notes')).not.toBeInTheDocument()
    expect(screen.queryByText('Archive')).not.toBeInTheDocument()
  ...
  describe('view note count chips', () => {
    ...
    it('aligns tag count pills to the same trailing column as view rows', () => {
      const tagged = mockEntries.map((entry, index) => (
        index === 0 ? { ...entry, properties: { ...entry.properties, tags: ['ai'] } } : entry
      ))
      render(
        <Sidebar entries={tagged} selection={defaultSelection} onSelect={() => {}} views={mockViews} />,
      )

      const tagItem = screen.getByText('ai').closest('[class*="cursor-pointer"]') as HTMLElement
      const tagCount = within(tagItem).getByTestId('sidebar-count-chip')
      ...
      expect(tagItem).toHaveStyle({ padding: '4px 8px 4px 12px' })
      expect(viewItem).toHaveStyle({ padding: '6px 8px 6px 12px' })
      expect(tagCount).toHaveStyle({
        background: 'var(--muted)',
        height: '18px',
        padding: '0 6px',
      })
