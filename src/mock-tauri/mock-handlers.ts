...
export const mockHandlers: Record<string, (args: any) => any> = {
  ...
  upload_image_via_picgo: (args: { filename: string }) => {
    return `https://cdn.example.mock/${encodeURIComponent(args.filename)}`
  },
  upload_image_path_via_picgo: (args: { source_path: string }) => {
    const filename = args.source_path.split(/[\\/]/u).pop() ?? 'image.png'
    return `https://cdn.example.mock/${encodeURIComponent(filename)}`
  },
  ...
  save_settings: (args: { settings: Settings }) => {
    ...
    mockSettings = {
      ...
      picgo_server_url: s.picgo_server_url ?? null,
      picgo_server_token: s.picgo_server_token ?? null,
    }
    return null
  },
