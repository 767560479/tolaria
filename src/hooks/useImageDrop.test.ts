...
describe('uploadImageFile', () => {
  ...
  it('passes file to Tauri save_image in Tauri mode', async () => {
    ...
    vi.mocked(invoke).mockImplementation(async (cmd: string) => {
      if (cmd === 'get_settings') {
        return { picgo_server_url: null, picgo_server_token: null }
      }
      if (cmd === 'save_image') {
        return '/vault/attachments/123-test.png'
      }
  ...
  it('uploads via PicGo when a server URL is configured', async () => {
    tauriMode = true
    const { invoke } = await import('@tauri-apps/api/core')
    vi.mocked(invoke).mockImplementation(async (cmd: string) => {
      if (cmd === 'upload_image_via_picgo') {
    ...
    await expect(
      uploadImageFile(file, '/vault', {
        picgoServerUrl: 'http://127.0.0.1:36677/upload',
        picgoServerToken: null,
      }),
    ).resolves.toBe('https://cdn.example/pic.png')
    expect(invoke).toHaveBeenCalledWith('upload_image_via_picgo', {
  ...
  it('does not fall back to local attachments when PicGo upload fails', async () => {
    tauriMode = true
    const { invoke } = await import('@tauri-apps/api/core')
    vi.mocked(invoke).mockRejectedValue(new Error('PicGo upload failed: denied'))
    ...
    ).rejects.toThrow('PicGo upload failed: denied')
...
describe('useImageDrop — Tauri native drag-drop', () => {
  ...
  it('copies native image drops into the vault and emits attachment asset URLs', async () => {
    ...
    vi.mocked(invoke).mockImplementation(async (cmd: string) => {
      ...
      if (cmd === 'copy_image_to_vault') {
        return '/vault/attachments/123-photo.png'
      }
      throw new Error(`Unexpected command: ${cmd}`)
    })
