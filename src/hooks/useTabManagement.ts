...
function upsertTab(
  ...
  const existingIndex = tabsRef.current.findIndex((tab) => notePathsMatch(tab.entry.path, nextTab.entry.path))
  const nextTabs = existingIndex >= 0
    ? tabsRef.current.map((tab, index) => (index === existingIndex ? nextTab : tab))
    : [...tabsRef.current, nextTab]
  tabsRef.current = nextTabs
  setTabs(nextTabs)
}

function replaceActiveTabEntry(
  tabsRef: React.MutableRefObject<Tab[]>,
  setTabs: React.Dispatch<React.SetStateAction<Tab[]>>,
  activePath: string | null,
  nextTab: Tab,
) {
  if (!activePath || notePathsMatch(activePath, nextTab.entry.path)) {
    upsertTab(tabsRef, setTabs, nextTab)
    return
  }
  const nextTabs = tabsRef.current.map((tab) => (
    notePathsMatch(tab.entry.path, activePath) ? nextTab : tab
  ))
  if (!nextTabs.some((tab) => notePathsMatch(tab.entry.path, nextTab.entry.path))) {
...
function openBinaryEntry(options: {
  ...
  navSeqRef.current += 1
  syncActiveTabPath(activeTabPathRef, setActiveTabPath, entry.path)
  upsertTab(tabsRef, setTabs, { entry, content: '' })
...
async function loadTextEntry(options: Required<Pick<NavigateToEntryOptions, 'forceReload'>> & NavigateToEntryOptions) {
  ...
  try {
    ...
    if (replaceActive) {
      replaceActiveTabEntry(tabsRef, setTabs, previousActivePath, { entry, content })
    } else {
      upsertTab(tabsRef, setTabs, { entry, content })
    }
  } catch (err) {
...
export function useTabManagement(options: TabManagementOptions = {}) {
  // Multi-note tabs: zero or more open notes; activeTabPath selects the focused one.
  ...
  /** Open a note — adds or focuses a tab. */
  ...
  const handleCloseTab = useCallback((path: string) => {
    const remaining = tabsRef.current.filter((tab) => !notePathsMatch(tab.entry.path, path))
    tabsRef.current = remaining
    setTabs(remaining)
    if (!notePathsMatch(activeTabPathRef.current, path)) return
    const nextPath = remaining.length > 0 ? remaining[remaining.length - 1].entry.path : null
