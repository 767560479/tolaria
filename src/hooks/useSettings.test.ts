...
import { useSettings, normalizeSettings } from './useSettings'
...
const defaultSettings: Settings = {
  ...
  note_list_folder_recursive: null,
...
describe('useSettings', () => {
  ...
  it('saves settings via backend', async () => {
    ...
    const expectedSettings = normalizeSettings(newSettings)

    await act(async () => {
      await result.current.saveSettings(newSettings)
    })

    expect(mockInvokeFn).toHaveBeenCalledWith('save_settings', { settings: expectedSettings })
  ...
  it('saves settings through native invoke when Tauri globals are not detectable', async () => {
    ...
    expect(nativeInvoke).toHaveBeenCalledWith('save_settings', { settings: expectedSettings })
