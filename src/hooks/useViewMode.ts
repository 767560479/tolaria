...
export function useViewMode(initialOverride?: ViewMode) {
  ...
  // `all` is sidebar + editor; the middle note list opens on demand (views/types/tags/filters)
  // or when the user explicitly chooses editor-list.
  const noteListVisible = viewMode === 'editor-list'
