...
describe('streamAiModel', () => {
  ...
  it('uses a request-scoped event channel for native model streams', async () => {
    ...
    invokeMock.mockImplementation(async (_command: string, args: { request: { event_name: string } }) => {
      // Events may arrive before invoke resolves; text still comes from the return value.
      handler?.({ payload: { kind: 'TextDelta', text: 'stale-event-text' } })
      handler?.({ payload: { kind: 'Done' } })
      return `reply:${args.request.event_name}`
    })
    ...
    expect(callbacks.onText).toHaveBeenCalledWith(`reply:${listenedEventName}`)
    expect(callbacks.onText).not.toHaveBeenCalledWith('stale-event-text')
    expect(callbacks.onDone).toHaveBeenCalledTimes(1)
    expect(unlisten).toHaveBeenCalledTimes(1)
  })

  it('applies invoke return text when Tauri stream events have not been delivered yet', async () => {
    isTauriState.value = true
    const unlisten = vi.fn()
    listenMock.mockResolvedValue(unlisten)
    // Real bug: invoke resolves before TextDelta/Done events flush on the JS event loop.
    invokeMock.mockResolvedValue('hello from model')

    const callbacks = createCallbacks()

    await streamAiModel({ provider, model, message: 'hello', callbacks })

    expect(callbacks.onText).toHaveBeenCalledWith('hello from model')
    expect(callbacks.onDone).toHaveBeenCalledTimes(1)
