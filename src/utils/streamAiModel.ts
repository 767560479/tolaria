...
import type { AgentStreamCallbacks } from './aiStreamCallbacks'
...
function streamMockAiModel({ provider, model, message, callbacks, signal }: StreamAiModelRequest): void {
...
function handleNativeStreamEvent(
  ...
  // TextDelta/Done are applied from the invoke return value. Batch model replies emit
  // those events immediately before invoke resolves, so relying on them races and can
  // finalize an empty assistant message ("finished without returning a reply").
  if (data.kind === 'TextDelta' || data.kind === 'Done') return
...
async function streamNativeAiModel(request: StreamAiModelRequest): Promise<void> {
  ...
  const unlisten = await listen<AiModelStreamEvent>(eventName, (event) => {
    handleNativeStreamEvent(event.payload, request.callbacks)
  })

  try {
    const text = await invoke<string>('stream_ai_model', {
      request: nativeAiModelRequest(request, eventName),
    })
    if (text.trim()) {
      request.callbacks.onText(text)
    }
    closeStream()
  } catch (err) {
