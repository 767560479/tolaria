...
function createMockUpdaterResult(
  checkForUpdates: () => Promise<{ kind: 'up-to-date' } | { kind: 'available'; version: string; displayVersion: string; htmlUrl: string } | { kind: 'error'; message: string }> = async () => ({ kind: 'up-to-date' }),
) {
  return {
...
describe('App', () => {
  ...
  it('renders the four-panel layout', async () => {
    render(<App />)
    expect(await screen.findByText('FOLDERS', {}, { timeout: 5000 })).toBeInTheDocument()
  })
  ...
  it('routes right-panel AI chat messages to the selected default agent', async () => {
    ...
    await waitFor(() => {
      expect(streamAiModel).toHaveBeenCalled()
    })
  })
  ...
  it('hides Inbox when explicit organization is disabled in vault config', async () => {
