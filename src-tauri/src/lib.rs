...
fn with_desktop_entry_plugins(builder: tauri::Builder<tauri::Wry>) -> tauri::Builder<tauri::Wry> {
    let builder = if vault_instance::is_separate_vault_instance() {
        ...
        builder.plugin(tauri_plugin_single_instance::init(|app, argv, _cwd| {
            // Shell "Open with" .md → spawn an independent vault instance so the
            // ordinary main process keeps its current vault / shared active_vault.
            if open_markdown::spawn_shell_markdown_vault_instance_from_args(&argv) {
                return;
            }
...
fn setup_app(app: &mut tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(desktop)]
    {
        ...
        // Only vault-instance processes should open shell Markdown in-process.
        // Ordinary launches delegate to a new instance before the UI starts.
        if vault_instance::is_separate_vault_instance() {
            open_markdown::emit_open_markdown_from_current_args(app.handle());
        }
    }
...
macro_rules! app_invoke_handler {
    () => {
        tauri::generate_handler![
            ...
            commands::upload_image_via_picgo,
            commands::upload_image_path_via_picgo,
            ...
            open_markdown::take_pending_shell_markdown_open,
...
pub fn run() {
    ...
    let builder = tauri::Builder::default()
        ...
        .manage(open_markdown::PendingShellMarkdownOpen::default());
