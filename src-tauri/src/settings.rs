...
pub struct Settings {
    ...
    pub picgo_server_url: Option<String>,
    pub picgo_server_token: Option<String>,
}
...
fn normalize_settings(settings: Settings) -> Settings {
    Settings {
        ...
        picgo_server_url: crate::vault::normalize_picgo_server_url(
            settings.picgo_server_url.as_deref(),
        ),
        picgo_server_token: crate::vault::normalize_picgo_server_token(
            settings.picgo_server_token.as_deref(),
        ),
    }
}
...
mod tests {
    ...
    fn test_settings_json_roundtrip() {
        let settings = Settings {
            ...
            picgo_server_url: Some("https://pic.example/upload".to_string()),
            picgo_server_token: Some("token".to_string()),
        };
    ...
    #[test]
    fn test_picgo_server_settings_are_normalized() {
        let loaded = save_and_reload(Settings {
            picgo_server_url: Some(" https://pic.example/upload ".to_string()),
            picgo_server_token: Some("  secret  ".to_string()),
            ..Default::default()
        });
        assert_eq!(
            loaded.picgo_server_url.as_deref(),
            Some("https://pic.example/upload")
        );
        assert_eq!(loaded.picgo_server_token.as_deref(), Some("secret"));

        let invalid = save_and_reload(Settings {
            picgo_server_url: Some("ftp://pic.example/upload".to_string()),
            picgo_server_token: Some("   ".to_string()),
            ..Default::default()
        });
        assert!(invalid.picgo_server_url.is_none());
        assert!(invalid.picgo_server_token.is_none());
    }
