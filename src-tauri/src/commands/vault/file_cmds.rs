...
#[tauri::command]
pub async fn upload_image_via_picgo(
    server_url: String,
    token: Option<String>,
    filename: String,
    data: String,
    ...
    tokio::task::spawn_blocking(move || {
        vault::upload_image_bytes_via_picgo(
            &server_url,
            token.as_deref(),
            &filename,
            &data,
    ...
    .await
    .map_err(|error| format!("PicGo upload task failed: {error}"))?
}

#[tauri::command]
pub async fn upload_image_path_via_picgo(
    server_url: String,
    token: Option<String>,
    source_path: PathBuf,
) -> Result<String, String> {
    tokio::task::spawn_blocking(move || {
        vault::upload_image_path_via_picgo(
            &server_url,
            token.as_deref(),
