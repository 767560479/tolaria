...
On Windows, NSIS also registers Tolaria as an Alternate “Open with” handler for `.md` files ([ADR-0177](./adr/0177-windows-shell-open-markdown-spawns-vault-instance.md), superseding [ADR-0176](./adr/0176-windows-shell-open-markdown-as-parent-vault.md)). Ordinary and secondary launches that receive a Markdown path spawn a separate `--tolaria-vault-instance` process for the file’s parent directory (with the note path on argv) instead of switching the main window’s vault. The instance emits `tolaria-open-markdown`; `useOpenMarkdownFromShell` opens the note there. Shared `active_vault` is preserved when the instance saves the vault list, so closing the window leaves the ordinary app on the previous vault.
