...
On Windows, NSIS also registers Tolaria as an Alternate “Open with” handler for `.md` files ([ADR-0181](./adr/0181-shell-open-registered-vault-and-skipped-scan-dirs.md), [ADR-0177](./adr/0177-windows-shell-open-markdown-spawns-vault-instance.md)). Ordinary and secondary launches that receive a Markdown path spawn a separate `--tolaria-vault-instance` process instead of switching the main window’s vault. If the file sits inside a registered vault, that vault is the instance root and `relative_note` may be nested; otherwise the file’s parent directory is the root. The instance emits `tolaria-open-markdown`; `useOpenMarkdownFromShell` opens the note there without a second full `reload_vault`. Shared `active_vault` is preserved when the instance saves the vault list. Vault scans, folder trees, and keyword search skip hidden directories plus `node_modules` / `target` / `dist` / `build` / `__pycache__` / `venv`, and do not follow directory symlinks.
...
| AI (workspace + commit drafts) | Direct model providers via `stream_ai_model` / `ai_models.rs` | - |
...
## Default layout (sidebar + editor + outline)
...
- **Editor** (flex, fills remaining space): Multiple notes may be open as tabs (`EditorTabBar` + `useTabManagement`, ADR-0179 superseding ADR-0003). Tab right-click offers Close / Close others / Close all (ADR-0180). Breadcrumb bar with filename controls, read-only legacy display-title context when a no-H1 note's title differs from its filename, word count, rich-editor width toggle, and the secondary-overflow Table of Contents action, BlockNote rich text editor with wikilink support, Markdown-compatible inline/display math rendering, editable Obsidian/GFM-style blockquote callouts, first-class Mermaid diagram blocks, sandboxed fenced HTML blocks, markdown-safe formatting controls, and schema-backed fenced code block highlighting via `@blocknote/code-block` plus lazy direct `@shikijs/langs` registrations for missing common grammars. Callout bodies retain rich inline marks and links, known aliases use semantic app colors, unknown types use neutral styling, and `+`/`-` fold markers control only the initial disclosure state while remaining unchanged in Markdown. Sandboxed HTML blocks resolve renderer-owned `{{...}}` vault expressions against current-note properties, external note scalar properties, single sheet cells, raw body-line references, and `json(...)` structured-data helpers before the existing sanitizer/iframe boundary. Scripts are blocked unless the fence explicitly declares `scripts="sandboxed"`, which grants only opaque-origin inline script execution while keeping same-origin, network, workers, forms, nested frames, parent DOM access, Tauri IPC, and top navigation unavailable. Can toggle to diff view (modified files), raw CodeMirror view, or a wide rich-editor reading surface with preserved side margins; raw CodeMirror remains full-width and unaffected by note width mode. Standalone HTML/HTM files use the same editor surface for an opaque-origin `HtmlFilePreview`: active content and remote passive loads are removed, vault-contained local resources are rewritten to scoped asset URLs, and the existing breadcrumb source action or `Cmd/Ctrl+\` opens CodeMirror for direct editing. Their breadcrumb keeps file/source/export actions while suppressing Markdown-only favorite, organized, neighborhood, width, table-of-contents, and archive controls. Raw CodeMirror chooses syntax highlighting from the file extension for Markdown, YAML, JSON, Python, SQL, JavaScript, and TypeScript files, highlights HTML inside fenced `html` blocks, and keeps unknown text files plain. Inline rich-editor images open in a localized shadcn lightbox on double-click while normal single-click BlockNote selection remains untouched, and tiny tracking-style images are ignored. Binary image, audio, video, and PDF files render through `FilePreview` as ordinary vault files using Tauri asset URLs; editor-embedded audio and video use the same scoped asset sources through the CSP `media-src` allow-list, while packaged PDF previews require scoped asset sources in both `object-src` and `frame-src` because the webview PDF renderer uses a nested frame context. Linux AppImage builds ask the native runtime whether audio/video should fall back to external-open controls before mounting webview media elements. External-open actions call `open_vault_file_external` so the target is validated against the active vault before the native default app opens it. Unsupported/broken binaries show explicit fallback states and keyboard focus returns to the note list on `Escape`. Decomposed into `Editor` (orchestrator), `EditorContent`, `HtmlFilePreview`, `FilePreview`, `EditorRightPanel`, `TableOfContentsPanel`, `SingleEditorView`, with hooks `useDiffMode`, `useEditorFocus`, and `useEditorSave`, plus the `useRawMode`/`RawEditorView` pair for raw source editing. Rich BlockNote input and raw CodeMirror input both route typed `->`, `<-`, and `<->` through the shared `src/utils/arrowLigatures.ts` resolver so arrow ligatures stay consistent across mode switches while escaped ASCII sequences remain literal. Rich-editor Markdown input transforms for arrows, inline math, and `==highlight==` share one capture-phase `beforeinput` execution path in `src/components/richEditorInputTransform.ts` and are composed by `src/components/richEditorInputTransformExtension.ts`. Navigation history (Cmd+[/]) remains available alongside tabs.
  ...
  Rich code blocks wrap long lines, render CSS-labeled ProseMirror widgets at each logical line start so the gutter shares the code's layout and copy-safe document positions, scope Cmd/Ctrl+A to their source, and support Enter-after-fence plus Cmd/Ctrl+Shift+Backquote creation paths through `richEditorCodeBlockShortcutExtension.ts`.
...
- **AI workspace** (docked panel or native window): `AiWorkspace` owns multi-chat orchestration, sidebar tabs, model target picker, and dock/pop-out controls. Header/guidance chrome lives in `AiWorkspaceChrome`, edge-resize handles in `AiWorkspaceResizeHandles`, conversation metadata/settings persistence in `aiWorkspaceConversations`, and sizing/class/style helpers in `aiWorkspaceSizing`. `AiTargetModelPicker` lists configured local/API model targets from `settings.ai_model_providers`. The status-bar AI affordance opens this workspace. Side mode defaults to 360 px, clamps legacy and resized widths to a 320 px minimum, and keeps the model selector and settings action visible on one composer row. Pop-out mode opens a dedicated undecorated transparent Tauri webview window labeled `ai-workspace` and boots the lightweight `AiWorkspaceWindowApp` route instead of the full vault shell. Chat sessions reuse `AiPanelView` for transcript/composer rendering. AI workspace cross-window localStorage, BroadcastChannel, storage-event, and subscriber-set plumbing is owned by `createCrossWindowPersistedStore`; domain stores keep only sanitization, mutation, and native persistence behavior.
...
The AI workspace can also open in a separate Tauri window through `openAiWorkspaceWindow()`. That window uses `?window=ai-workspace` to boot the lightweight `AiWorkspaceWindowApp` route, receives the active vault context through URL params, opts out of main-window size constraints, and redocks by emitting `tolaria:ai-workspace-dock-requested` to the main window. Its webview is transparent so the rounded workspace shell defines the visible floating-window corners across desktop platforms.
...
In-app AI is limited to direct model providers (see [ADR-0175](adr/0175-fork-remove-mcp-and-cli-agents.md)). There is no bundled vault MCP server, WebSocket bridge, or CLI coding-agent subprocess.

### Surfaces

1. **AiWorkspace chat** — multi-conversation panel or native pop-out window; streams the selected `model:<provider>/<model>` target through `stream_ai_model`.
2. **Git commit-message drafts** — manual commit dialog and optional AutoGit checkpoints call `commitMessageDraft.ts`, which uses the same direct-model path when AI features and a configured model target are available.

### Direct model chat (AiWorkspace)

1. **Frontend** (`AiWorkspace`, `AiPanel`, `useCliAiAgent`, `aiAgentSession.ts`, `streamAiModel.ts`, `aiTargets.ts`) — conversation state, note-context assembly, model target selection, streaming transcript UI, and optional vault refresh after tool-created notes
2. **Backend** (`ai_models.rs`, `ai_model_tools.rs`) — HTTP streaming to OpenAI-compatible chat completions and Anthropic Messages endpoints (Ollama, LM Studio, OpenRouter, OpenAI, Anthropic, Gemini, custom compatible bases)
3. **Settings** — `settings.ai_model_providers` stores non-secret provider metadata; `settings.default_ai_target` holds the active `model:<provider>/<model>` id. Provider secrets live in `ai-provider-secrets.json` or referenced environment variables, not in `settings.json`

OpenAI-compatible targets may invoke Tolaria's narrow native `create_note` tool when an active vault is loaded. The tool uses the same create-only, active-vault-bounded write path as the UI and emits tool events so the renderer can refresh and open the created note. Models do not receive shell access or general file-write tools.

Each `stream_ai_model` call receives a request-scoped stream event name. The renderer subscribes to that scoped channel for the duration of the request and ignores stale events after stop or completion.

#### Model stream flow

```mermaid
sequenceDiagram
    participant U as User (AiWorkspace)
    participant FE as aiAgentSession (Frontend)
    participant R as ai_models.rs (Rust)
    participant P as Model provider

    U->>FE: sendMessage(text, references)
    FE->>FE: buildContextSnapshot(activeNote, linkedNotes)
    FE->>R: invoke('stream_ai_model', {provider, model_id, message, systemPrompt, vaultPath, eventName})
    loop Normalized stream
        R->>P: HTTP chat request
        P-->>R: token / tool events
        R-->>FE: emit(eventName, event)
        alt TextDelta
            FE->>FE: accumulate response
        else ToolStart / ToolDone
            FE->>FE: update tool cards; refresh vault on create_note
        else Done
            FE->>FE: finalize response
        end
    end
```

When a model creates a vault note through the native tool, `aiAgentFileOperations.ts` triggers `onFileCreated` so the workspace reloads affected entries.

### Context building

`ai-context.ts` builds a structured JSON snapshot from the active note and linked entries:
...
Large active notes are compacted into a head/tail body snapshot before they enter the prompt. The snapshot records `bodyTruncated` metadata so lower-context providers still receive useful context without exceeding practical prompt limits.

### Commit-message drafts

`useCommitFlow.generateCommitMessageForDialog()` reloads modified files with line stats and fills the commit textarea without invoking `git_commit`. When AI features are enabled and `default_ai_target` resolves to a configured model, `commitMessageDraft.ts` sends bounded path/status metadata plus capped `get_file_diff` excerpts to `stream_ai_model` and returns a concise imperative summary. Disabled AI, offline providers, oversized diffs, and model failures fall back to deterministic changed-file labels from `commitMessage.ts`. The command palette action "Generate Commit Message from Diff" uses the same path. AutoGit's `autogit_use_ai_commit_messages` setting opts automatic checkpoints into that AI draft path with the same fallback behavior.

### Provider authentication

Provider secrets are not written to `settings.json`. Hosted API targets use Tolaria's local app-data secrets file (`ai-provider-secrets.json`, outside vaults/worktrees and owner-only on Unix) or reference an environment variable name. Env-backed keys resolve from the app process first, then from exported values in the user's zsh/bash startup files on Unix so GUI-launched sessions can still use shell-managed secrets. Local endpoints can omit authentication.

`src/shared/aiModelProviderCatalog.json` is the shared source for provider defaults, local/API grouping, API-key environment placeholders, and runtime fallback base URLs; the renderer imports it through `aiTargets.ts`, and Tauri includes the same JSON in `ai_models.rs`.
...
`useGettingStartedClone` reuses the same parent-folder semantics for the status-bar / command-palette clone action.
...
4. Linux AppImage builds strip AppImage loader variables from system-git subprocesses before spawning them, keeping `git-remote-https` on the host library stack
...
| `picgo_upload.rs` | PicGo-compatible multipart upload (`upload_image_bytes_via_picgo` / `upload_image_path_via_picgo`) when Gallery settings provide an http(s) server URL |
...
| `ai_models.rs` | Direct model provider streaming, API-key resolution, and OpenAI/Anthropic-compatible HTTP clients |
| `ai_model_tools.rs` | Native `create_note` tool handling for OpenAI-compatible model streams |
...
| Command | Description |
|---------|-------------|
| `stream_ai_model` | Stream a configured local/API model through the normalized event layer |
| `save_ai_model_provider_api_key` | Persist a provider API key in local app-data secrets |
| `delete_ai_model_provider_api_key` | Remove a stored provider API key |
| `test_ai_model_provider` | Probe a provider/model configuration without starting a workspace chat |
...
| `upload_image_via_picgo` | Upload base64 image bytes to a PicGo-compatible http(s) endpoint; returns the remote URL |
| `upload_image_path_via_picgo` | Read a local image path and upload it to a PicGo-compatible http(s) endpoint; returns the remote URL |
...
| `useCliAiAgent` | `messages`, `status`, tool actions | Selected model-target conversation backed by `aiAgentSession` + `stream_ai_model` |
...
| `useSettings` | App settings (telemetry, release channel, theme mode, UI language, date display format, auto-sync interval, Git visibility, AutoGit thresholds, AI model providers/target, Gitignored-content visibility, All Notes file visibility) | Persistent settings |
...
`.github/workflows/release.yml` publishes Windows-only unsigned NSIS installers on two refs (ADR 0178):

```
push to alpha
  → shallow checkout + tags
  → compute calendar alpha version YYYY.M.D-alpha.N
    and tag alpha-vYYYY.M.D-alpha.NNNN
  → restore pnpm / Rust / NSIS caches
  → windows-latest: stamp, build NSIS, publish GitHub prerelease

push tag stable-vYYYY.M.D
  → stamp version YYYY.M.D
  → restore caches, build NSIS, publish GitHub stable release

push to main → no release (Stable baseline only)
```

Cold builds spend most wall time in `pnpm tauri build` (~11 min); CI caches Cargo `target` and Tauri NSIS tools so subsequent Alpha pushes reuse compiled dependencies.
macOS and Linux installers are not produced by this pipeline. See ADR 0173 / 0178.

### Versioning

- Alpha builds stamp `YYYY.M.D-alpha.N` and display as `Alpha YYYY.M.D.N`; tags zero-pad as `alpha-vYYYY.M.D-alpha.NNNN`.
- Stable builds stamp and display `YYYY.M.D` from tag `stable-vYYYY.M.D`.
- If a stable tag already uses today's UTC date, alpha advances to the next calendar day before assigning `-alpha.N` so Alpha remains semver-newer than Stable across channel switches.
- The workflow stamps the computed version into `tauri.conf.json` and `Cargo.toml` at build time.

### In-App Updates

This fork does **not** auto-download or auto-install updates (no Tauri updater signatures).

```
Command Palette → Check for Updates
  → check_for_app_update (GitHub Releases API for 767560479/tolaria)
    → idle (no newer release) → toast only
    → available → UpdateBanner with "Download on GitHub"
      → opens the release html_url in the browser
...
In-app updates are **manual GitHub Releases checks** only (no Tauri updater plugin, no auto-install):

- `useUpdater()` + `check_for_app_update` query `767560479/tolaria` Releases (including prereleases)
- `UpdateBanner` opens the release page in the browser when a newer build exists
- Download the unsigned NSIS `*-setup.exe` from GitHub Releases yourself
...
```
#[cfg(desktop)]  — git CLI, menu bar, direct model AI, updater
...
- Direct model streaming (`stream_ai_model`)
- Menu state updates
