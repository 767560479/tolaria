...
- `useFolderActions()` composes `useFolderRename()`, `useFolderDelete()`, `useFolderMove()`, and `useFolderDuplicate()` to keep folder mutations selection-aware while the rest of `App.tsx` only wires the resulting callbacks into `Sidebar` and the command registry.
...
`useAiWorkspaceWindowBridgeEvents()` owns native AI-workspace event subscriptions and listener cleanup for popped-out workspace windows.
...
- **Generate commit message command**: Cmd+K action that opens the commit dialog, reloads the selected repository with line stats, and fills the message field. Configured model targets receive structured path/status/line-count metadata plus bounded diff excerpts via `stream_ai_model`; disabled AI, offline providers, or model failures use a deterministic changed-file summary instead.
...
- `createImeCompositionKeyGuardExtension()` intercepts composing `Enter` and `Space` keydown events before BlockNote's list shortcuts see them, so Korean/Japanese/Chinese IMEs can commit text in list items without Tolaria splitting or duplicating the current item. It stops editor shortcut propagation only; it does not prevent the browser/IME default composition action.
...
Raw CodeMirror keeps Markdown mode active for `.md` notes, uses the HTML language support for both standalone `.html`/`.htm` files and fenced `html` block contents, and handles `Tab` as a literal editor insertion instead of browser focus traversal.
...
- Persists vault list to `$XDG_CONFIG_HOME/com.tolaria.app/vaults.json`, defaulting to `$HOME/.config/com.tolaria.app/vaults.json` on Unix platforms. App config path policy is declared in `src-tauri/resources/app-config-policy.json` and consumed by the Rust app helper: reads check the current Tolaria namespace, then legacy `com.laputa.app`, first in the preferred config root and then in the platform config root when it differs; writes target the current namespace in the preferred root.
...
- Settings: zoom, view mode, editor mode, tag colors, status colors, property display modes, Inbox/All Notes note-list column overrides, explicit organization workflow toggle, Git setup prompt preference
...
Tolaria tracks managed vault-level guidance separately from normal note content:
- `AGENTS.md` is the canonical managed guidance file for vault conventions
- `CLAUDE.md` is a compatibility shim that points Claude Code back to `AGENTS.md`
- `GEMINI.md` is an optional Antigravity/Gemini compatibility shim
...
- `restore_vault_ai_guidance` repairs only Tolaria-managed files and creates the optional Antigravity/Gemini shim on explicit request; user-authored custom guidance files are surfaced as custom and left untouched
...
These files are for human authors and external tools. Tolaria no longer spawns CLI coding agents that consume them in-app.
...
- On Linux AppImage launches, every system-git command removes AppImage loader overrides such as `LD_LIBRARY_PATH`, `LD_PRELOAD`, and `GIT_EXEC_PATH` before spawning, so helpers like `git-remote-https` bind against the host library stack instead of Tolaria's bundled WebKit/AppImage libraries
...
App-level settings persisted at `$XDG_CONFIG_HOME/com.tolaria.app/settings.json`, defaulting to `$HOME/.config/com.tolaria.app/settings.json` on Unix platforms. `settings.json` and `vaults.json` share the same `src-tauri/resources/app-config-policy.json` search order used by Rust:
...
interface Settings {
  ...
  release_channel: string | null // retained for telemetry; not shown in update UI
  automatic_update_checks_enabled: boolean | null // forced false on save; auto-check removed
  ...
  default_ai_target: string | null // "model:<provider>/<model>"
...
- **AI model sessions** — `ai_agent_message_sent`, `ai_agent_message_blocked`, `ai_agent_response_completed`, `ai_agent_response_failed`, `ai_agent_response_stopped`, and related workspace events use only coarse provider/target categories and status metadata. `ai_workspace_open` includes only whether a model selector is available; model identifiers are never sent.
...
### Hooks
- **`useUpdater()`** — Manual update check state machine (checking / available / error). No startup auto-check and no in-app download/install. Available updates open the GitHub release URL in the browser.
...
### Frontend helpers
- **`src/lib/releaseChannel.ts`** — Still used by telemetry identify; update checks no longer select a channel feed.
- **`src/lib/appUpdater.ts`** — Invokes `check_for_app_update` and holds the GitHub Releases page fallback URL.

### Rust
- **`src-tauri/src/app_updater.rs`** — Fetches `https://api.github.com/repos/767560479/tolaria/releases`, picks the newest non-draft `alpha-v…` tag, compares against `CARGO_PKG_VERSION`, and returns metadata including `htmlUrl` when newer.
...
### Tauri Commands
- **`check_for_app_update`** — Manual GitHub Releases comparison; returns `null` when up to date.

### CI/CD
- **`.github/workflows/release.yml`** — Windows-only unsigned release on push to `main`. Computes calendar alpha version `YYYY.M.D-alpha.N` and tag `alpha-vYYYY.M.D-alpha.NNNN`, builds NSIS via `pnpm tauri build --target x86_64-pc-windows-msvc --bundles nsis` with `createUpdaterArtifacts: false`, and publishes the `*-setup.exe` as a GitHub prerelease. No signing secrets are required (`GITHUB_TOKEN` only).
