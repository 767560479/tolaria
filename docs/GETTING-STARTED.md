...
Official releases build unsigned NSIS installers only:

```bash
pnpm tauri build --target x86_64-pc-windows-msvc --bundles nsis
```

No Tauri updater or Authenticode secrets are required. Windows SmartScreen may warn on the installer; choose More info → Run anyway to install. In-app auto-update is not supported for these builds.
...
Pushing to `main` triggers `.github/workflows/release.yml` on GitHub Actions. It builds an
unsigned Windows NSIS installer (`x86_64-pc-windows-msvc`) and publishes a prerelease on
GitHub Releases. No Tauri, Authenticode, or Apple secrets are required. Windows SmartScreen
may warn on unsigned installers; choose More info → Run anyway to install.

Quality checks still run locally via husky and on `.github/workflows/ci.yml`.
...
Linux AppImage builds still use the user's system `git`. Before Tolaria spawns Git subprocesses, it removes AppImage loader overrides such as `LD_LIBRARY_PATH`, `LD_PRELOAD`, and `GIT_EXEC_PATH` so HTTPS clone helpers use the host library stack instead of bundled AppImage libraries.
...
│   │   ├── AiPanel.tsx           # AI transcript/composer surface (selected model target)
│   │   ├── AiMessage.tsx         # AI message display
│   │   ├── AiActionCard.tsx      # Tool action cards during model streams
...
│   │   ├── useCliAiAgent.ts      # Selected model-target session state + streaming pipeline
│   │   ├── useAiAgentPreferences.ts # Default model-target persistence
...
│   │   ├── ai-agent.ts           # Legacy agent stream utilities (commit/chat helpers)
│   │   ├── ai-chat.ts            # Token estimation utilities
│   │   ├── ai-context.ts         # Context snapshot builder
│   │   ├── streamAiModel.ts      # Frontend wrapper for stream_ai_model
...
│   │
│   ├── lib/
│   │   ├── aiTargets.ts          # Model target ids + provider catalog wiring
...
│   │   ├── ai_models.rs          # Direct model provider streaming + secrets
│   │   ├── ai_model_tools.rs     # Native create_note tool for model streams
...
| `src-tauri/src/lib.rs` | Tauri setup, command registration, startup tasks. |
...
| `src-tauri/src/ai_models.rs` | Direct model provider streaming, API-key resolution, and provider HTTP clients. |
| `src-tauri/src/ai_model_tools.rs` | Native `create_note` tool handling for OpenAI-compatible model streams. |
...
| File | Why it matters |
|------|---------------|
| `src/components/AiWorkspace.tsx` | Multi-chat AI workspace orchestration — chat sessions, sidebar tabs, model target controls, and dock/pop-out wiring. |
...
| `src/components/AiPanel.tsx` | Reusable AI transcript/composer surface — selected model target, reasoning, tool actions, and streaming state. |
| `src/utils/openAiWorkspaceWindow.ts` | Native Tauri AI workspace window creation, focus, and dock-back traffic-light handling. |
| `src/hooks/useCliAiAgent.ts` | React owner for the selected model-target session state. |
| `src/lib/aiAgentSession.ts` | Message/session lifecycle for prompt normalization, history, streaming via `stream_ai_model`, and reset behavior. |
| `src/lib/aiTargets.ts` | Model target ids, provider catalog wiring, and default target resolution. |
| `src/utils/streamAiModel.ts` | Frontend invoke/listen wrapper for `stream_ai_model`. |
| `src/utils/commitMessageDraft.ts` | AI commit-message drafts via `stream_ai_model` with deterministic fallback. |
...
| File | Why it matters |
|------|---------------|
| `src/hooks/useSettings.ts` | App settings (telemetry, release channel, theme mode, UI language, date display format, Git visibility, auto-sync interval, default note width, sidebar type pluralization, AI model providers/target). |
...
| `src/components/SettingsPanel.tsx` | Settings UI for telemetry, release channel, Git visibility, sync interval, UI language, content display preferences, AI model providers/target, and the vault-level explicit organization toggle. |
...
### Work with AI (direct models)

1. **Workspace chat**: Edit `src/components/AiWorkspace.tsx`, `src/components/AiPanel.tsx`, and `src/lib/aiAgentSession.ts` for conversation UX and streaming behavior.
2. **Model targets/settings**: Edit `src/lib/aiTargets.ts`, `src/components/AiAgentModelPicker.tsx`, `src/hooks/useAiAgentPreferences.ts`, and `src/components/SettingsPanel.tsx`. Targets use `model:<provider>/<model>` ids stored in `settings.default_ai_target` with provider metadata in `settings.ai_model_providers`.
3. **Backend streaming**: Edit `src-tauri/src/ai_models.rs` and `src-tauri/src/ai_model_tools.rs` for provider HTTP clients, secrets, and the native `create_note` tool.
4. **Frontend invoke wrapper**: Edit `src/utils/streamAiModel.ts` when stream event shapes or invoke args change.
5. **Commit-message drafts**: Edit `src/utils/commitMessageDraft.ts` and `src/hooks/useCommitFlow.ts` for AI-generated Git commit messages (also uses `stream_ai_model`).
